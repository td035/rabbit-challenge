...
問：RNNとword2vecの違いを簡潔に述べよ。
解答：RNNは時系列データを扱うのに適したニューラルネットワークで、単語の分散表現ベクトルを得るための手法。
...
問：seq2seqとAttentionの違いを簡潔に述べよ。
解答：seq2seqは、1つの時系列データから別の時系列データを得るための手法。Attension Mechanismは、時系列データの中身に対して関連性に重みをつける手法。
